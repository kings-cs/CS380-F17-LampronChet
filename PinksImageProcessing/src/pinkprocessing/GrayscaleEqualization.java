...
public class GrayscaleEqualization {
	...
	public int[] calculateIdealizedHistogram(int[] cumulativeFrequencyResult, int numOfPixels)
		...
		int idealizedValue = numOfPixels / cumulativeFrequencyResult.length;
		 int[] histogram = new int[cumulativeFrequencyResult.length]; //int[] dimensions
		 /* = { numOfPixels, cumulativeFrequencyResult.length, histogram.length };
		 * Pointer ptrHistogram = Pointer.to(histogram); Pointer ptrDimensions =
		 ...
		 * histogram.length, ptrHistogram, null); cl_mem memDimensions =
		 * CL.clCreateBuffer(deviceManager.getContext(), CL.CL_MEM_READ_ONLY |
		 * CL.CL_MEM_COPY_HOST_PTR, Sizeof.cl_float * dimensions.length, ptrDimensions,
		 * null);
		 * 
		 * File kernelFile = new
		 * File("Kernels/GrayscaleEqualization/calculateIdealHistogram"); Scanner
		 * kernelScan = new Scanner(kernelFile); StringBuffer sourceBuffer = new
		 * StringBuffer(); while (kernelScan.hasNextLine()) {
		 * sourceBuffer.append(kernelScan.nextLine()); sourceBuffer.append("\n"); }
		 * cl_program program = CL.clCreateProgramWithSource(deviceManager.getContext(),
		 * 1, new String[] { sourceBuffer.toString() }, null, null);
		 * 
		 * CL.clBuildProgram(program, 0, null, null, null, null);
		 * 
		 * long[] globalWorkSize = new long[] { cumulativeFrequencyResult.length };
		 * long[] localWorkSize = new long[] { workSize }; deviceManager.createQueue();
		 * cl_kernel calculateKernel = CL.clCreateKernel(program,
		 * "calculate_ideal_histogram", null);
		 * 
		 * CL.clSetKernelArg(calculateKernel, 0, Sizeof.cl_mem,
		 * Pointer.to(memHistogram)); CL.clSetKernelArg(calculateKernel, 1,
		 * Sizeof.cl_mem, Pointer.to(memDimensions)); double startTime =
		 * System.nanoTime(); CL.clEnqueueNDRangeKernel(deviceManager.getQueue(),
		 * calculateKernel, 1, null, globalWorkSize, localWorkSize, 0, null, null);
		 * calculatedRuntime += System.nanoTime() - startTime;
		 * 
		 * CL.clEnqueueReadBuffer(deviceManager.getQueue(), memHistogram, CL.CL_TRUE, 0,
		 * histogram.length * Sizeof.cl_int, ptrHistogram, 0, null, null);
		 * 
		 * CL.clReleaseKernel(calculateKernel); CL.clReleaseProgram(program);
		 * CL.clReleaseMemObject(memHistogram); CL.clReleaseMemObject(memDimensions);
		 * 
		 * kernelScan.close();
		 */
